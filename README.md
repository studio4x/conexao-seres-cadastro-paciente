...
Adultos sem responsável mantêm somente as linhas completas do atendimento, sem duplicar os próprios dados pessoais. Quando existe responsável, as labels ficam compactas (`Paciente`, `CPF`, `Nasc.`, `Contato`, `Endereço`, `Nasc. resp.`, `Atendimento`, `Modo`, `1ª sessão`, `Modo 1ª sessão` e `Mídia`), preservando integralmente os valores aplicáveis. Se a composição ultrapassar o orçamento, o backend interrompe o fluxo antes de qualquer consulta ou criação no Asaas, registra apenas tamanho e orçamento e retorna mensagem controlada; não há truncamento silencioso.
