...
Na leitura das observações do cliente, o webhook reconhece tanto o ordinal original `1ª sessão` / `Modo 1ª sessão` quanto a variante ASCII `1a sessão` / `Modo 1a sessão`, mantendo validações estritas da data, do horário e da modalidade. Campos não reconhecidos continuam vazios e geram diagnóstico técnico sem registrar o conteúdo das observações.
...
7. para cliente existente, inclusive quando recuperado a partir de uma cobrança anterior, valida se o CPF do cliente vinculado corresponde ao CPF enviado. Se o cadastro estiver marcado como removido, tenta restaurá-lo por `/customers/{id}/restore` antes de prosseguir. Depois faz a atualização em duas etapas independentes: primeiro endereço fiscal e depois `observations`; campos opcionais vazios, como complemento, não são enviados no update;
...
O workflow específico e importável da Jornada está em `n8n/conexao-seres-jornada-yoga-v1.5.json`. Ele recebe os eventos `jornada_yoga_registration_created` e `jornada_yoga_payment_paid`, valida o token Bearer e o `eventId`, deduplica os disparos por `paymentId` e envia ao participante as mensagens aprovadas de WhatsApp e e-mail. A validação do link de pagamento usa expressão regular compatível com o sandbox do Code node, sem depender do global `URL`. O envio reutiliza a instância `ConexaoSeres` da Evolution API e o SMTP `cadastro@conexaoseres.com.br`, conforme o workflow clínico de referência. O JSON é importado desativado; após configurar `N8N_CONEXAO_SERES_JORNADA_WEBHOOK_TOKEN` e conferir as credenciais no n8n, deve ser ativado manualmente.
