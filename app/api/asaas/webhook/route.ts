...
function isFirstSessionPayment(payment: JsonRecord) {
...
async function processConfirmedPayment(baseUrl: string, apiKey: string, payment: JsonRecord) {
  ...
  if (fiscalInfo.status < 200 || fiscalInfo.status >= 300) {
    logAsaasFailure("Asaas fiscal information lookup failed", fiscalInfo, { paymentId, customerId, externalReference });
  ...
  if (servicesResult.status < 200 || servicesResult.status >= 300) {
    logAsaasFailure("Asaas municipal service lookup failed", servicesResult, { paymentId, customerId, externalReference });
  ...
  if (invoiceResult.status < 200 || invoiceResult.status >= 300) {
    logAsaasFailure("Asaas invoice creation failed", invoiceResult, { paymentId, customerId, externalReference });
...
export async function POST(request: Request) {
  ...
  if (!expectedToken || expectedToken === "COLE_AQUI_O_TOKEN_DO_WEBHOOK_ASAAS") {
    return NextResponse.json({ message: "Webhook fiscal não configurado." }, { status: 503 });
  }
  ...
  if (event !== "PAYMENT_CONFIRMED") return NextResponse.json({ received: true, processed: false }, { status: 200 });
  ...
  if (!isFirstSessionPayment(payment as JsonRecord)) {
    console.info("Asaas payment webhook ignored: not a confirmed first-session payment");
    return NextResponse.json({ received: true, processed: false }, { status: 200 });
  }

  const apiKey = (env.ASAAS_API_KEY as string | undefined)?.trim() || "";
  if (!apiKey) return NextResponse.json({ message: "Integração Asaas não configurada." }, { status: 503 });
  const baseUrl = ((env.ASAAS_API_URL as string | undefined) || "https://api.asaas.com/v3").replace(/\/$/, "");
  const result = await processConfirmedPayment(baseUrl, apiKey, payment as JsonRecord);
