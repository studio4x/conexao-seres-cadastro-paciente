...
Para a primeira sessão, a configuração fiscal da conta permanece no Asaas, mas a emissão da cobrança avulsa deve ser solicitada explicitamente pelo webhook fiscal próprio após o pagamento. As implementações são `app/api/asaas/webhook/route.ts` e `cpanel-server/api/asaas-webhook.php`, publicadas em `POST /api/asaas/webhook` e autenticadas pelo cabeçalho `asaas-access-token` com `ASAAS_WEBHOOK_TOKEN`. Aceitam `PAYMENT_CONFIRMED` somente com status `CONFIRMED` e `PAYMENT_RECEIVED` somente com status `RECEIVED` ou `RECEIVED_IN_CASH`, incluindo PIX e recebimento em dinheiro registrado manualmente; ignoram demais eventos com 2xx.

O webhook fiscal deve validar `payment.id`, `payment.customer`, valor `230` e a referência exata `cs-paciente-<24 hex>-sessao-1`. Antes do POST, deve consultar `GET /v3/invoices?payment=<paymentId>`; se não houver nota, consultar `/v3/fiscalInfo/` e percorrer `/v3/fiscalInfo/services` paginado por `offset`, com limite máximo 100, até o fim indicado por `hasMore`/`totalCount`. Deve priorizar código municipal exatamente `04510`; na ausência de código, aceitar somente descrição contendo `04510`, `4.08` e `Terapia Ocupacional`. Uma única correspondência usa o ID retornado; múltiplas são ambíguas e não geram NF; nenhuma usa o código municipal confirmado `04510` e nome padrão, sem inventar ID. Depois executa `POST /v3/invoices` vinculado ao payment. O payload deve usar `externalReference` com sufixo `-nfse`, valor 230, deduções zero, data efetiva em `America/Sao_Paulo`, `updatePayment=false`, `retainIss=false` e ISS de 2%, com retenções adicionais somente quando comprovadas pela configuração fiscal. O PHP deve usar lock por hash do payment com `flock`; no runtime Edge, a consulta ao Asaas é a fonte compartilhada de idempotência.
