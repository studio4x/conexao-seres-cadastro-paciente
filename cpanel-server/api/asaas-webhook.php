...
function process_confirmed_payment(string $baseUrl, string $apiKey, array $payment): bool
{
...
if ($expectedToken === '' || $expectedToken === 'COLE_AQUI_O_TOKEN_DO_WEBHOOK_ASAAS') {
    respond(['message' => 'Webhook fiscal não configurado.'], 503);
}
...
if (!is_array($payment) || !is_first_session_payment($payment)) {
    error_log('Asaas payment webhook ignored: not a confirmed first-session payment.');
    respond(['received' => true, 'processed' => false], 200);
}

$apiKey = trim((string) (getenv('ASAAS_API_KEY') ?: ($fileConfig['asaas_api_key'] ?? '')));
if ($apiKey === '' || $apiKey === 'COLE_AQUI_A_CHAVE_DA_API_DO_ASAAS') {
    respond(['message' => 'Integração Asaas não configurada.'], 503);
}
if (!function_exists('curl_init')) {
    respond(['message' => 'A integração fiscal não está disponível no servidor.'], 503);
}

$baseUrl = rtrim((string) (getenv('ASAAS_API_URL') ?: ($fileConfig['asaas_api_url'] ?? 'https://api.asaas.com/v3')), '/');
$retry = process_confirmed_payment($baseUrl, $apiKey, $payment);
if ($retry) {
