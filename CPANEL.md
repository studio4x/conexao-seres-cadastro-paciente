...
O formulário envia também `serviceType`, `entryType` e `attendanceMode`. O PHP valida esses códigos conforme a idade da pessoa atendida, grava os labels nas observações do novo cliente e mantém `entryType` exclusivo para menores e `attendanceMode` exclusivo para adultos. O campo obrigatório `referralSource` registra como a pessoa conheceu a Conexão Seres, aceitando somente as opções da lista suspensa e gravando o rótulo selecionado em `observations`. Os campos nativos do titular (nome, CPF/CNPJ, e-mail, celular, endereço, número, complemento, bairro/província e CEP) seguem no payload do cliente. Por solicitação do fluxo, `observations` também registra para o paciente `Idade`, `CPF`, `Sexo`, `Nasc.`, `Celular`, `E-mail` e `Endereço` completo em uma única linha. Sexo e data de nascimento, que não aparecem como campos do cadastro de cliente na referência do Asaas, são preservados nas observações; quando houver responsável, a cidade/UF do responsável também é registrada. Para menores, contato e endereço próprios continuam ausentes porque o formulário não os solicita. Como o Asaas não publica `maxLength` para `customer.observations`, o PHP aplica o orçamento interno conservador de 550 bytes UTF-8 antes de qualquer chamada ao Asaas, sem truncamento. Esses campos são informativos nesta fase e não alteram a cobrança de R$ 230,00 nem a emissão da NFS-e.
...
O parser do PHP aceita `1ª sessão`/`Modo 1ª sessão` e as variantes ASCII `1a sessão`/`Modo 1a sessão` ao ler as observações retornadas pelo Asaas. Os formatos de data, horário e modalidade continuam validados; o payload não inventa dados ausentes.


O endpoint `api/asaas-webhook.php` encaminha ao n8n as cobranças válidas da primeira sessão nos eventos `PAYMENT_CONFIRMED` e `PAYMENT_RECEIVED`, respeitando os status já validados pelo webhook fiscal. O payload inclui `paymentId` para que o workflow faça a deduplicação por cobrança. O nome e o telefone são consultados diretamente no Asaas por `GET /v3/customers/{customerId}`; `customerName` continua sendo o nome do titular e `customerWhatsapp` prioriza `mobilePhone` e usa `phone` quando necessário, permanecendo vazio se ambos faltarem. A mesma resposta do cliente é usada para extrair de `observations` `patientName`, `firstSessionDate`, `firstSessionTime` e `firstSessionMode`; o parser aceita as labels compactas atuais e as labels anteriores, incluindo `Online via Google Meet` e o rótulo legado `Online`. Quando a linha de pessoa atendida não existe, `patientName` usa `customerName`; linha inválida e dados ausentes ou alterados resultam em strings vazias sem bloquear o pagamento ou a NFS-e. Não é feita uma segunda consulta de cliente. Também são enviados `invoiceNumber` e `invoiceUrl`, primeiro aproveitando os campos do evento e, se algum faltar, consultando `GET /v3/payments/{paymentId}`. A URL nunca é construída manualmente; campos ausentes permanecem como strings vazias sem interromper o n8n ou a NFS-e.
...
Para preparar a inscrição e a emissão fiscal, o formulário exige:

- data de nascimento de participante adulto;
- origem da inscrição em “Como ficou sabendo da Jornada?”; ao selecionar `Outro`, um campo de texto complementar passa a ser obrigatório;
- CEP;
- logradouro;
- número;
...
Quando o cliente já existe, o backend consulta o cadastro atual e confere se o CPF do cliente vinculado corresponde ao CPF enviado no formulário. Cobranças antigas podem continuar apontando para um cliente removido; nesse caso, se a API retornar `deleted: true`, o backend tenta restaurar o mesmo cliente por `POST /v3/customers/{id}/restore` antes de atualizá-lo. Depois, a atualização é feita em duas chamadas `PUT /v3/customers/{id}`: a primeira somente com os dados fiscais do endereço e a segunda somente com `observations`. Complemento vazio é omitido. O fluxo diferencia falhas de restauração, divergência de CPF, endereço e observações.
