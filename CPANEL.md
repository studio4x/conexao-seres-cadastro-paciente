...
O formulário envia também `serviceType`, `entryType` e `attendanceMode`. O PHP valida esses códigos conforme a idade da pessoa atendida, grava os labels nas observações do novo cliente e mantém `entryType` exclusivo para menores e `attendanceMode` exclusivo para adultos. Os campos nativos do titular (nome, CPF/CNPJ, e-mail, celular, endereço, número, complemento, bairro/província e CEP) seguem no payload do cliente. Por solicitação do fluxo, `observations` também registra para o paciente `Idade`, `CPF`, `Sexo`, `Nasc.`, `Celular`, `E-mail` e `Endereço` completo em uma única linha. Sexo, data de nascimento, cidade e UF, que não aparecem como campos do cadastro de cliente na referência do Asaas, são preservados nas observações para o paciente e, quando houver responsável, para a cidade/UF do responsável. Para menores, contato e endereço próprios continuam ausentes porque o formulário não os solicita. Como o Asaas não publica `maxLength` para `customer.observations`, o PHP aplica o orçamento interno conservador de 500 bytes UTF-8 antes de qualquer chamada ao Asaas, sem truncamento. Esses campos são informativos nesta fase e não alteram a cobrança de R$ 230,00 nem a emissão da NFS-e.
