...
Quando o cliente já existe, o backend consulta o cadastro atual e preserva outras observações já existentes. A atualização é feita em duas chamadas `PUT /v3/customers/{id}`: a primeira somente com os dados fiscais do endereço e a segunda somente com `observations`. Complemento vazio é omitido do update, seguindo a recomendação do provedor de enviar apenas campos efetivamente alterados. Assim, o fluxo diferencia `JOURNEY_ADDRESS_UPDATE_FAILED` de `JOURNEY_OBSERVATIONS_UPDATE_FAILED`. O conteúdo combinado das observações continua respeitando o orçamento interno conservador de 550 bytes UTF-8, sem truncar dados anteriores.
