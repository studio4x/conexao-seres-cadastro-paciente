...
O formulário envia também `serviceType`, `entryType` e `attendanceMode`. O PHP valida esses códigos conforme a idade da pessoa atendida, grava os labels nas observações do novo cliente e mantém `entryType` exclusivo para menores e `attendanceMode` exclusivo para adultos. Para adulto sem responsável, as observações contêm somente os dados do atendimento; para adulto com responsável e menor, preservam também a identificação da pessoa atendida em labels compactas. Como o Asaas não publica `maxLength` para `customer.observations`, o PHP aplica o orçamento interno conservador de 500 bytes UTF-8 antes de qualquer chamada ao Asaas, sem truncamento. Esses campos são informativos nesta fase e não alteram a cobrança de R$ 230,00 nem a emissão da NFS-e.
